...
ttestpvals <- function(df, splitvar, ttestformula) {  
  ...
  p.ttest = cbind(coD[ ,c('estimate','var')],
                  ...
                  n.ttest)
  mycolnames = c('cohen.D','cohenD.se','tstat','df',"raw", "fdr", "bonferroni","n")
...
FApooled_healthyavg[order(FApooled_healthyavg$V1),]
...
alleffects_FA$cohen.D <- as.numeric(alleffects_FA$cohen.D)
alleffects_FA$cohenD.se <- as.numeric(alleffects_FA$cohenD.se)
...
alleffects_FA$cohen.D
tmp <- subset(alleffects_FA,site=='pooled')
alleffects_FA_order <- as.character(tmp[order(as.numeric(tmp$cohen.D)),'Tract'])
...
ggplot(subset(alleffects_FA,site=='pooled'), aes(y=as.numeric(cohen.D), x=Tract)) +
  geom_errorbar(aes(ymin = (cohen.D - cohenD.se), ymax =  (cohen.D + cohenD.se)), width = 0.2) +
  geom_point(size = 8, shape = 18) + 
  geom_point(data=subset(alleffects_FA,site!='pooled'), aes(y=cohen.D, x=Tract, colour = site), size = 3) +
  geom_hline(yintercept=0) +
  scale_y_reverse() + 
  labs(x='',y="Group Effect Size (Hedges g)") +
  coord_flip()